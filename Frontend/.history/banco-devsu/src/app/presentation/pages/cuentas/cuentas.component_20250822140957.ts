...
export class CuentasComponent implements OnInit {
  ...
  cargarCuentas(): void {
    this.cuentaService.getCuentas().subscribe(
      (response: any) => {
        if (response.success) {
          this.cuentas = response.data.map((cuenta: CuentaDTO) => ({
            cuentaId: cuenta.cuentaId,
            tipo: cuenta.tipo === 1 ? 'Crédito' : 'Débito',
            valor: cuenta.valor,
            saldo: cuenta.saldo,
            fecha: new Date(cuenta.fecha).toLocaleDateString('es-ES', {
              year: 'numeric',
              month: '2-digit',
              day: '2-digit'
            })
          }));
          console.log("json cuentas :", JSON.stringify(this.cuentas));
          this.filteredItems = [...this.cuentas];

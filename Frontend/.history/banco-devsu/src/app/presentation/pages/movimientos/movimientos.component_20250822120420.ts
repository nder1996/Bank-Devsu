      ...
      (response: any) => {
        if (response.success) {
          ...
          this.movimientos = response.data.map((movimiento: MovimientoDTO) => ({
            ...movimiento,
            cuenta: movimiento.cuenta, // Cambiado de cuentaId a cuenta
            tipoMovimiento: movimiento.tipoMovimiento === TipoMovimiento.Credito ? 'Crédito' : 'Débito', // Ajustada la lógica para usar TipoMovimiento correctamente
            tipoMovimiento: movimiento.tipoMovimiento === 1 ? 'Crédito' : 'Débito', // Ajustada la lógica para usar TipoMovimiento correctamente

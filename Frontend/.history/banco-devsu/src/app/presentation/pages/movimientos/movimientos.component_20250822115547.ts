...
export class MovimientosComponent implements OnInit {
  ...
  cargarMovimientos(): void {
    this.movimientoService.getMovimientos().subscribe(
      (response: any) => {
        if (response.success) {
          console.log("movimientos "+JSON.stringifyresponse(response.data))
          this.movimientos = response.data.map((movimiento: any) => ({
            ...movimiento,
            cuenta: movimiento.cuentaId, // Asignar el número de cuenta
            tipoMovimiento: movimiento.tipoMovimiento === 1 ? 'Crédito' : 'Débito', // Corregir lógica para tipo de movimiento
